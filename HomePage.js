    ...
    for (let i = 0; i < mainDiv.querySelectorAll(".message").length; i++) {
        ...
        reviewElements.forEach((reviewElement, j) => {
            messageData[`id${i}`].rates[`id${j}`] = {
                ...
                raterDescription: reviewElement.querySelector(`#reterDescription`).textContent,
...
fetch("/Frontend/src/HomePage/myJsFiles/MyData.html")
    ...
    .then(data => {
        ...
        // You can now work with the message0 and message1 objects
...
// }
    ...
    showAllOption1.addEventListener("click", () => {
        ...
        for (let i = 0; i < Object.keys(message0).length; i++){
            // creating the cards...
            createCards(0, i, 3)
    ...
    showAllOption2.addEventListener("click", () => {
        ...
        // looping throw all the data from "myData.js"
        for (let i = 0; i < Object.keys(message1).length; i++){
            // creating the cards...
            createCards(1, i, 4)
            // 0 for section 1, the cards of "رسائل الماجستير"
            // i to loop throw all the data
...
for (let i = 0; i < 2; i++){
   ...
   const createCards = (i, j, myId) => {
    ...
    myOuterDiv.setAttribute('data-target', `mainDiv-${i}`);

    
    // creating the image of the cards
    let myImage = this.document.createElement("img");
    // setting its value from "myData.js"
    myImage.src = eval(`message${i}.id${j}`).imageSrc;
    ...
    let myAdjustedDescription = eval(`message${i}.id${j}`).description.split("").splice(0, 43).join(""); 
    ...
    myStartsRate.innerHTML =  eval(`message${i}.id${j}`).rate;
    ...
    myWriterName.innerHTML = eval(`message${i}.id${j}`).writerName;
